...
Those five are the core. A verified finding then becomes **canon** — it has a lifecycle: pin it with a text-normalized prompt hash via [`lock`](./running-it/#pin-a-dispatch-for-replay), and if it is later retracted, [`withdraw` / `requalify`](./running-it/#roll-back-a-withdrawn-finding) roll it back across every dependent.
